# Quickstart: Validating the Gallery Hero section
...
1. Start the dev server and open the Gallery page in a browser at desktop width (≥1280px).
2. Confirm the Hero renders as the first section at the top of the page (after global
   navigation).
3. Compare against the provided design screenshot: the "THE TRANSFORMATIONS" eyebrow label,
   the headline "See the difference," and the supporting paragraph ("Real spaces, real
   results. Every project starts with chaos and ends with calm."), all center-aligned, over a
   cream background with two large soft circular shapes at the top-right and left-middle edges.
...
## 3. Paragraph wrap (Edge case)

1. At the default desktop width, confirm the supporting paragraph wraps to two lines ("Real
   spaces, real results. Every project starts with chaos and ends with / calm.") with
   consistent line spacing and no cramped or overlapping lines, as shown in the screenshot.
