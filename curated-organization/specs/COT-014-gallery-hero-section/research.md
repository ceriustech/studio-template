# Phase 0 Research: Gallery Hero section
...
## Decision: Follow the Services Hero's prop-less `hero` component shape (not the paired `Name.tsx`/`Name.types.ts` convention)

**Decision**: The `Hero` component (`app/routes/pages/gallery/components/hero/index.tsx`)
is a lowercase-folder, `index.tsx`-only component with no `.types.ts` file, mirroring the
already-merged Services-page `hero` component exactly.

**Rationale**: Both hero components are the same shape of problem — a single, static,
prop-less section with fixed copy and no per-consumer variation. The Services-page `hero`
already establishes this precedent in the merged codebase; introducing a different shape
(PascalCase folder + paired types file, as `Footer`/`Intro`/`Testimonial` use) for this
component alone would create two competing conventions for the identical case with no
behavioral difference to justify it.

**Alternatives considered**:

- *Create `Hero/Hero.tsx` + `Hero.types.ts` per the general component convention* —
  rejected: there are no props to type (static content only), and doing so would diverge from
  the direct precedent already set by the Services Hero, for no functional benefit.
...
- *Accept eyebrow/headline/paragraph as props for future reuse* — rejected: there is exactly
  one consumer (`gallery`) and one instance of this section; adding a props surface for
...
pseudo-elements (matching the mockup's own approach) — no `<img>`, no extra DOM nodes exposed
to assistive tech.
...
article and the Media article's scope (this isn't an image/video asset). This also matches the
approach already taken for the Services Hero's identical decorative circles.
...
**Decision**: The headline ("See the difference") renders as an `<h1>`, since the Hero is the
first section on the Gallery page and the mockup itself uses `<h1>` for it.
...
## Decision: Decorative circle placement mirrors the mockup exactly (top-right and left-middle), not the Services Hero's placement

**Decision**: The `::before` circle sits at `top: -300px; right: -200px` (600×600px) and the
`::after` circle sits at `top: 70px; left: -200px` (500×500px), per
`.gallery-hero::before`/`::after` in the mockup — a mirrored layout from the Services Hero's
own circles (which sit top-left and bottom-right).

**Rationale**: The mockup for the Gallery page defines its own distinct circle positions,
different from the Services Hero's. Faithfully translating the Gallery mockup's coordinates
(rather than reusing the Services Hero's coordinates) is required to match the provided design
screenshot exactly, per the spec's Acceptance Criteria.

**Alternatives considered**:

- *Reuse the Services Hero's circle coordinates* — rejected: would not match the Gallery
  page's design screenshot, which shows the circles in different corners.
